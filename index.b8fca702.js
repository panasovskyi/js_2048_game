...
function renderTable() {
    ...
    for(let i = 0; i < 4; i++){
        ...
        for(let j = 0; j < 4; j++){
            ...
            if (value !== 0) cells[j].classList.add(`field-cell--${value}`);
        }
    }
}
...
window.addEventListener("keyup", (e)=>{
    const gameStatus = game.getStatus();
    if (gameStatus === "playing") switch(e.key){
        case "ArrowLeft":
            game.moveLeft();
            break;
        case "ArrowRight":
   ...
   */ constructor(initialState){
    ...
    addRandomTile() {
        ...
        for(let y = 0; y < this.size; y++){
            for(let x = 0; x < this.size; x++)if (!board[y][x]) emptyTileCoords.push([
                y,
                x
            ]);
        }
        if (!emptyTileCoords.length) return;
    ...
    transpose(board) {
        ...
        for(let i = 0; i < board.length; i++)for(let j = 0; j < board[i].length; j++)transposedBoard[i].push(board[j][i]);
        return transposedBoard;
    }
    noMovesCheck() {
        const board = this.state;
        const allTiles = board.flat();
        if (allTiles.includes(0)) return false;
        for(let i = 0; i < this.size; i++)for(let j = 0; j < this.size; j++){
            if (j < 3 && board[i][j] === board[i][j + 1] || i < 3 && board[i][j] === board[i + 1][j]) return false;
        }
        return true;
    }
    postMoveCheck(isMoved) {
        const victory = this.state.flat().includes(2048);
        if (victory) this.status = "win";
        if (isMoved && !victory) this.addRandomTile();
        if (this.noMovesCheck()) this.status = "lose";
    }
    movement(direction, transpose = false) {
        ...
        for(let i = 0; i < board.length; i++){
            ...
            if (direction === "right" || direction === "down") row.reverse();
            board[i] = row;
            if (!moved && JSON.stringify(initialRow) !== JSON.stringify(row)) moved = true;
        }
